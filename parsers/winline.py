...
class WinlineParser(BaseBookmakerParser):
    bookmaker_name = "Winline"
    sample_file = "sample_winline.json"

    @property
    def source_url(self) -> str:
        return self.settings.winline_api_url
    ...
    async def fetch_events(self) -> list[dict[str, Any]]:
        ...
        raise ParserError(
            self.bookmaker_name,
            "Unsupported Winline payload. Configure WINLINE_API_URL to return normalized events or adapt parser mapping.",
            self.source_url,
            "payload_error",
        )
