...
`FONBET_API_URL` также поддерживает сырой публичный ответ FONBET вида:

```text
https://line-.../ma/events/event?lang=ru&version=0&eventId=...&scopeMarket=1600
```

Из такого ответа парсер берёт корневое событие по `eventId`, команды, лигу, время, `1X2`, форы и основной тотал. Один такой URL соответствует одному событию; для полноценного мониторинга нескольких матчей нужен список событий или турнирный endpoint.
